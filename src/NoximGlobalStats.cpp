...
double NoximGlobalStats::getCriticalPathExecutionTime(){
	if (NoximGlobalParams::taskgraph_mode){
		vector<int> parent_nodes = noc->tg.getParentNodes();
		vector<int> leaf_nodes = noc->tg.getLeafNodes();
		
		double start_time=-1;
		double end_time=-1;

		for(vector<int>::iterator it_parent_nodes=parent_nodes.begin(); it_parent_nodes!=parent_nodes.end(); it_parent_nodes++){
			if (noc->t[(*it_parent_nodes)/NoximGlobalParams::mesh_dim_x][(*it_parent_nodes)%NoximGlobalParams::mesh_dim_y]->pe->first_packet_transmit_time < start_time || start_time==-1)
				start_time = noc->t[(*it_parent_nodes)/NoximGlobalParams::mesh_dim_x][(*it_parent_nodes)%NoximGlobalParams::mesh_dim_y]->pe->first_packet_transmit_time/1000;
		}
		cout<<"start time: "<<start_time<<endl;
		for(vector<int>::iterator it_leaf_nodes=leaf_nodes.begin(); it_leaf_nodes!=leaf_nodes.end(); it_leaf_nodes++){
			if (noc->t[(*it_leaf_nodes)/NoximGlobalParams::mesh_dim_x][(*it_leaf_nodes)%NoximGlobalParams::mesh_dim_y]->pe->first_packet_process_time > end_time || end_time==-1)
				end_time = noc->t[(*it_leaf_nodes)/NoximGlobalParams::mesh_dim_x][(*it_leaf_nodes)%NoximGlobalParams::mesh_dim_y]->pe->first_packet_process_time/1000;
		}
...
void NoximGlobalStats::showStats(std::ostream & out, bool detailed)
{
    ...
    if (NoximGlobalParams::taskgraph_mode){
       out<<"Critical path execution time: "<<getCriticalPathExecutionTime() <<endl;
    }
}
