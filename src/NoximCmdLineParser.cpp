...
void showHelp(char selfname[])
{
    ...
    cout <<
	"\t-tg FILENAME\tEnable taskgraph mode (can only be used with table based traffic)"<<endl;
    cout <<
	"\t-tm MODULE_FILENAME INPUT_FILENAME\tEnable task mapping mode (Don't use use with taskgraph mode or with custom traffic mode)"<<endl<<endl;
    cout <<
...
void showConfig()
{
    ...
    cout << "- taskgraph_mode = " << (NoximGlobalParams::taskgraph_mode?"yes":"no")<<endl;
    cout << "- taskmapping_mode = " << (NoximGlobalParams::taskmapping_mode?"yes":"no")<<endl;
...
void checkInputParameters()
{
    ...
    if (NoximGlobalParams::taskgraph_mode && NoximGlobalParams::traffic_distribution!=TRAFFIC_TABLE_BASED){
	cerr << "Error: taskgraph mode can only be used with -traffic table"<<endl;
	exit(1);
    }
    if (NoximGlobalParams::taskmapping_mode && (NoximGlobalParams::traffic_distribution!=DEFAULT_TRAFFIC_DISTRIBUTION || NoximGlobalParams::taskgraph_mode==true)){
	cerr << "Error : taskmapping mode cannot be used with custom traffic rules or taskgraph mode"<<endl;
	exit(1);
    }
}
...
void parseCmdLine(int arg_num, char *arg_vet[])
{
	...
	for (int i = 1; i < arg_num; i++) {
	    ...
	    else if (!strcmp(arg_vet[i], "-tg")){
		    NoximGlobalParams::taskgraph_mode=true;
	    	strcpy(NoximGlobalParams::taskgraph_filename, arg_vet[++i]);
	    }
	    else if (!strcmp(arg_vet[i], "-tm")){
		    NoximGlobalParams::taskmapping_mode=true;
		    strcpy(NoximGlobalParams::taskmapping_module_filename, arg_vet[++i]);
		    strcpy(NoximGlobalParams::taskmapping_input_filename, arg_vet[++i]);
	    }
	    else {
